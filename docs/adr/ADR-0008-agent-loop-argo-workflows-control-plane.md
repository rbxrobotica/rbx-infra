...
| `mission-gate-wait` | `delivered → approved\|rejected` | Poll-wait against maestro's gate status until a human gate decision is recorded (`gate_decisions` / `approval_requests`). The engine never decides — it waits (P4). *(Precision added in rollout step 2: originally drafted as a suspend node resumed by a maestro callback, but a callback would require maestro to write to the kube-apiserver, which maestro must not do — polling keeps the pull model pure: nothing outside the cluster engine pushes into it, and maestro keeps zero cluster permissions.)* |
| `mission-outcome` | `approved\|rejected\|stopped → completed` | Records `rejected` or `stopped` + `stop_reason` (nine canonical reasons) in maestro and ends. For `approved` missions it poll-waits until maestro relays the **merge event** (GitHub webhook/poll — merge itself is human, P4, outside the engine), then records the terminal **`completed`** close-out: artifacts sealed (P10) and the branch-cleanup notification emitted (P2) as a maestro event, never as a git operation by the engine. |
