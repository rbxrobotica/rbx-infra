...
| `rbx-ia-br` | `rbx-commerce-sandbox-secrets` | Role-managed: `DATABASE_URL`, `COMMS_API_URL`, `ASAAS_API_KEY`, `ASAAS_WEBHOOK_TOKEN`. Satwake extensions pending provisioning: `ALTCHA_SECRET`, `COMMS_SERVICE_API_KEY`, `COMMERCE_PUBLIC_TENANT_ID` | Role-managed values: `rbx/commerce-sandbox/db-password`, `rbx/commerce-sandbox/asaas-api-key`, `rbx/commerce-sandbox/asaas-api-webhook-token`. Satwake extension source of truth must be established before use. |
...
properties. Provision the three Satwake extensions from an approved durable
source before deploying sandbox checkout. A retired extension remains present
