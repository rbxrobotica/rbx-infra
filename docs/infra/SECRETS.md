...
| `rbx-ia-br` | `rbx-commerce-sandbox-secrets` | Role-managed: `DATABASE_URL`, `COMMS_API_URL`, `ASAAS_API_KEY`, `ASAAS_WEBHOOK_TOKEN`. Satwake secret extensions pending provisioning: `ALTCHA_SECRET`, `COMMS_SERVICE_API_KEY` | Role-managed values: `rbx/commerce-sandbox/db-password`, `rbx/commerce-sandbox/asaas-api-key`, `rbx/commerce-sandbox/asaas-api-webhook-token`. Separate sandbox sources for the Satwake extensions must be established before use. |
...
The `k8s-secrets` role refreshes only its four managed properties in an existing
`rbx-commerce-sandbox-secrets` Secret. It preserves separately provisioned
Satwake properties, but a fresh Secret still contains only the four managed
properties. Provision the two Satwake secret extensions from approved durable
sources before deploying sandbox checkout. A retired extension remains present
until it is explicitly removed; the role does not prune it.
...
The role currently sets sandbox `COMMS_API_URL` to the production Comms service.
Adding its existing service key to the sandbox would let public sandbox
checkout requests send real verification email through production Postmark.
Select an isolated test Comms endpoint and credential, or explicitly approve
that external send path, before provisioning `COMMS_SERVICE_API_KEY`. This
documentation does not provision credentials or enable checkout.
