      ...
      if ! git diff --quiet "origin/${base_branch}...HEAD" 2>/dev/null; then
        if git push -u origin "mission/${code}" >>"${log_file}" 2>&1; then
          pr_url=$(gh pr create --base "${base_branch}" --head "mission/${code}" \
            ...
            --body "Auto-generated by rbx-agent-runner (Corbetti). Executor: ${executor} (${agent_model}). verify_command: \`${verify_cmd:-none}\` -> **${verify_status}** (exit ${verify_exit_code}). Review before merge — merge is human (P4)." \
            2>>"${log_file}")
