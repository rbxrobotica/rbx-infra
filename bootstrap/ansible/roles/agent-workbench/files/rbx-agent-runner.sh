...
report_delivered() {
  ...
  if printf '%s' "${payload}" | maestro_post "/missions/${code}/lease/state"; then
    ...
    printf 'DELIVERED input_tokens=%s output_tokens=%s verify=%s\n' \
      "${input_tok:-unknown}" "${output_tok:-unknown}" "${vstatus:-n/a}" \
      >>"${LOG_DIR}/${code}.log"
  else
...
"
  ...
  # ── persist + report terminal state ───────────────────────────────────────
  # Delivery means the review artifact exists, not merely that execution and
  # verify succeeded. Commit, push, and PR creation therefore precede the
  # delivered outcome; any persistence failure stops for operator review.
  local report_vexit=""
  if [[ "${verify_status}" != "not_run" ]]; then
    report_vexit="${verify_exit_code}"
  fi
  if [[ ${exit_code} -eq 0 && "${verify_status}" != "failed" ]]; then
    local branch="mission/${code}" persistence_status="failed"
    local persistence_reason="git_checkout_failed" pr_url="" commit_ready=true

    if git -C "${worktree}" checkout -b "${branch}" >>"${log_file}" 2>&1; then
      persistence_reason="git_add_failed"
      if git -C "${worktree}" add -A >>"${log_file}" 2>&1; then
        if git -C "${worktree}" diff --cached --quiet &&
           git -C "${worktree}" diff --quiet "origin/${base_branch}...HEAD"; then
          persistence_reason="no_diff"
        else
          persistence_reason="git_commit_failed"
          if ! git -C "${worktree}" diff --cached --quiet; then
            git -C "${worktree}" commit -q -m "mission ${code} (${mtype}, verify=${verify_status})

Auto-generated by rbx-agent-runner on Corbetti.
Executor: ${executor} (${agent_model}). verify_command: ${verify_cmd:-none} -> ${verify_status} (exit ${verify_exit_code}).
Review before merge — merge is human (P4, ADR-0019)." >>"${log_file}" 2>&1 ||
              commit_ready=false
          fi

          if [[ "${commit_ready}" == "true" ]] &&
             ! git -C "${worktree}" diff --quiet "origin/${base_branch}...HEAD" 2>/dev/null; then
            persistence_reason="git_push_failed"
            if git -C "${worktree}" push -u origin "${branch}" >>"${log_file}" 2>&1; then
              persistence_reason="pr_create_failed"
              if pr_url=$(cd "${worktree}" && gh pr create \
                --base "${base_branch}" --head "${branch}" \
                --assignee ldamasio \
                --title "mission ${code}: ${mtype}" \
                --body "Auto-generated by rbx-agent-runner (Corbetti). Executor: ${executor} (${agent_model}). verify_command: \`${verify_cmd:-none}\` -> **${verify_status}** (exit ${verify_exit_code}). Review before merge — merge is human (P4, ADR-0019). Assigned to operator per CEO directive 2026-07-27 (RUNNER-PUSH-001)." \
                2>>"${log_file}"); then
    ...
    if [[ "${persistence_status}" == "ready" && -n "${pr_url}" ]]; then
      log "PR ${code}: ${pr_url}"
      report_delivered "${code}" "${input_tok}" "${output_tok}" "${verify_status}" "${report_vexit}"
    else
      log "STOP ${code}: persistence=${persistence_reason}; no delivered outcome (see ${log_file})"
