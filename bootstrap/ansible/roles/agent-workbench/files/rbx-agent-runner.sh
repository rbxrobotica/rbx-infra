...
"
  ...
  # ── PR creation (C2.1): persist delivered work as a reviewable PR ─────────
  # Delivered missions push a mission branch + open a PR so the operator has a
  # review artifact (the human merge gate, P4). The runner (not the agent) does
  # this so delivery is deterministic. If verify passed but there's no diff vs
  # the base branch, the agent produced nothing reviewable — warn loudly, since
  # verify is necessary-not-sufficient (ADR-0019 risk). Must run BEFORE the
  # worktree is removed.
  if [[ ${exit_code} -eq 0 && "${verify_status}" != "failed" ]]; then
    (
      cd "${worktree}"
      git checkout -b "mission/${code}" >>"${log_file}" 2>&1 || true
      git add -A >>"${log_file}" 2>&1
      git commit -q -m "mission ${code} (${mtype}, verify=${verify_status})
...
Review before merge — merge is human (P4, ADR-0019)." >>"${log_file}" 2>&1 || true
      if ! git diff --quiet "origin/${base_branch}...HEAD" 2>/dev/null; then
        if git push -u origin "mission/${code}" >>"${log_file}" 2>&1; then
          pr_url=$(gh pr create --base "${base_branch}" --head "mission/${code}" \
            --assignee ldamasio \
            ...
            2>>"${log_file}")
          log "PR ${code}: ${pr_url:-FAILED to open}"
        else
