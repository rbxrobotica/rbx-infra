...
remote_dir="$1"
backup_dir="$2"
[[ -d "$remote_dir/config" ]] || { printf '[ERROR] Ergo config directory is absent.\n' >&2; exit 1; }
install -d -m 0700 "$backup_dir"
was_running=false
if docker inspect -f '{{.State.Running}}' rbx-ergo 2>/dev/null | grep -qx true; then
    was_running=true
fi
restart() {
    if [[ "$was_running" == true ]]; then
        cd "$remote_dir"
        docker compose --env-file .env -f compose.yaml start ergo >/dev/null
    fi
}
trap restart EXIT
if [[ "$was_running" == true ]]; then
    cd "$remote_dir"
    docker compose --env-file .env -f compose.yaml stop ergo >/dev/null
fi
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
archive="$backup_dir/ergo-$stamp.tar.gz"
